...
export function AboutHero({ eyebrow, title, description }: { eyebrow: string; title: string; description: string }) {
  return <section className="relative overflow-hidden bg-[var(--paper)]"><div className="absolute right-[-8rem] top-[-10rem] h-[28rem] w-[28rem] rounded-full border-[2.5rem] border-[var(--gold-100)]" aria-hidden="true" /><div className="section-shell relative grid gap-12 py-20 lg:grid-cols-[1.1fr_0.9fr] lg:items-end lg:py-28"><div className="max-w-4xl"><p className="eyebrow">{eyebrow}</p><h1 className="display-text mt-5 text-5xl leading-[1.03] text-[var(--forest-950)] sm:text-6xl lg:text-7xl">{title}</h1><p className="mt-8 max-w-2xl text-lg leading-8 text-[var(--muted)]">{description}</p></div><div className="relative ml-auto w-full max-w-xs"><Image src={assetPath("/brand/AAN_logo1.png")} alt="AAN Legacy Foundation logo" width={1080} height={1080} sizes="(min-width: 1024px) 20rem, 80vw" className="h-auto w-full" priority /></div></div></section>;
}
